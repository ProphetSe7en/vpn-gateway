...
**Latest version: `v1.4.4`** — [all tags](https://github.com/prophetse7en/vpn-gateway/pkgs/container/vpn-gateway)

```bash
docker pull ghcr.io/prophetse7en/vpn-gateway:v1.4.4
```
...
docker run -d \
  ...
  -e PRIVNET=192.168.0.0/24 \
  ghcr.io/prophetse7en/vpn-gateway:v1.4.4
```
...
**Or install manually:** Go to **Docker** → **Add Container**, set Repository to `ghcr.io/prophetse7en/vpn-gateway:v1.4.4`, and add the required paths, ports, and capabilities (see above).
