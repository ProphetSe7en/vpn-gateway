...
func TestParseClientIP_NoProxy(t *testing.T) {
	...
	r.RemoteAddr = "192.168.0.20:54321"
	r.Header.Set("X-Forwarded-For", "8.8.8.8")
	ip := ParseClientIP(r, nil)
	if ip.String() != "192.168.0.20" {
		t.Errorf("no trusted proxies: got %s, want 192.168.0.20 (ignore XFF)", ip)
	}
}
...
func TestValidateURL_Allowlist(t *testing.T) {
	err := ValidateURL("http://192.168.0.5:7878", nil)
	if err == nil {
		t.Error("expected error without allowlist")
	}
	allow := []net.IP{net.ParseIP("192.168.0.5")}
	err = ValidateURL("http://192.168.0.5:7878", allow)
	if err != nil {
